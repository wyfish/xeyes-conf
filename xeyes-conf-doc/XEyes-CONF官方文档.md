...
> 可参考项目 "xxl-conf-sample-spring"（接入XXl-CONF的示例项目，供用户参考学习），代码位置：IndexController.index() 
...
<bean id="demoConf" class="com.xxl.conf.sample.demo.DemoConf2">
    <constructor-arg index="0" value="#{T(XxlConfClient).get('key')}" />
    <property name="paramByXml" value="#{T(XxlConfClient).get('default.key03')}" />
</bean>
```

- 用法：占位符方式 "#{T(XxlConfClient).get('key')}"；
- 优点：
    - 配置从配置中心自动加载；
    - 存在LocalCache，不用担心性能问题；
